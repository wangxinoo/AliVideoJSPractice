var newfile=GenerateUploadFilename();var policyText = {"expiration": "2080-01-01T12:00:00.000Z",  "conditions": [ ["content-length-range", 0, 1048576000]  ]};accessid= 'LTAICdv31hjvk8BC';accesskey= 'Edhv5fo1cYoavtXuctJg3L3psI90mI';host = 'http://suncommunity.oss-cn-hangzhou.aliyuncs.com';ossurl:"http://img.nxygsq.com/";var policyBase64 = Base64.encode(JSON.stringify(policyText))
message = policyBase64
var bytes = Crypto.HMAC(Crypto.SHA1, message, accesskey, { asBytes: true }); var signature = Crypto.util.bytesToBase64(bytes); var uploader = new plupload.Uploader({ runtimes: 'html5,flash,silverlight,html4', browse_button: 'selectfiles', container: document.getElementById('container'), flash_swf_url: 'lib/plupload-2.1.2/js/Moxie.swf', silverlight_xap_url: 'lib/plupload-2.1.2/js/Moxie.xap', url: host, multipart_params: {'Filename': '${filename}',   'key':newfile+'${filename}','policy': policyBase64,  'OSSAccessKeyId': accessid,  'success_action_status' : '200', 'signature': signature,}, init: { PostInit: function() {
		...
		FilesAdded: function(up, files) {
			plupload.each(files, function(file) {
				...
				document.getElementById('ossfile').innerHTML += '<div id="' + file.id + '">'  + ' '  + '<b></b><img src="" width="100"/>'
		...
		FileUploaded: function(up, file, info) {
            ...
            if (info.status >= 200 || info.status < 200)
            {
                ...
                document.getElementById(file.id).getElementsByTagName('img')[0].src = file.name;
            }
            else
            {
