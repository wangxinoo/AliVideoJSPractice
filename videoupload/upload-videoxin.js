...
var policyText = {"expiration": "2080-01-01T12:00:00.000Z",  "conditions": [ ["content-length-range", 0, 1048576000]  ]};
accessid= 'LTAICdv31hjvk8BC';
accesskey= 'Edhv5fo1cYoavtXuctJg3L3psI90mI';
	 ...
	 multipart_params: {
		...
		FilesAdded: function(up, files) {
			...
			plupload.each(files, function(file) {
			    file.name = "http://img.nxygsq.com/" + newfile + file.name;
				//file.filename=GenerateUploadFilename();
				document.getElementById('ossfile2').innerHTML += '<div id="' + file.id + '">'  + ' '  + '<b></b><img src="" width="100"/>'
				+'<div class="progress"><div class="progress-bar" style="width: 0%"></div></div>'
				+'</div>';
				uploader2.start();
				return false;
	 ...
	 multipart_params: {
		...
		FilesAdded: function(up, files) {
			...
			plupload.each(files, function(file) {
			    file.name = "http://img.nxygsq.com/" + newfile + file.name;
				//file.filename=GenerateUploadFilename();
				document.getElementById('ossfile3').innerHTML += '<div id="' + file.id + '">'  + ' '  + '<b></b><img src="" width="100"/>'
				+'<div class="progress"><div class="progress-bar" style="width: 0%"></div></div>'
				+'</div>';
				uploader3.start();
				return false;
